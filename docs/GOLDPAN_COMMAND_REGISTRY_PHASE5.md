...
| 8 | CMD000008 | `intake.packet.commit_ingest` | automated_job | approved | missing | Type restored to `automated_job` (v0.1 original) — DEC000001 §5.7 does not redefine this command's type, only its role as the sole ordinary path to `ingested`. Once invoked it performs the durable write and status transition without continuous human interaction, matching the `automated_job` definition in §6; it was not reclassified as `mutation` by any Founder decision. **Precondition and failure-behavior contract now specified — DEC000003 (approved 2026-07-16):** single all-or-nothing transaction, fails closed with SQLSTATE `GP422` on an unresolved allergen disclosure (leaving `packet_status` unchanged), no `partially_ingested` state. See §22 and `docs/decisions/DEC000003_CANONICAL_INTAKE_EVIDENCE_INGESTION_ARCHITECTURE.md`. Not yet built. |
...
**Transaction boundary and failure behavior — DEC000003 (approved 2026-07-16):** single atomic transaction; every approved, ingestion-bound evidence element (after excluding anything resolved by an Evidence Disposition) must be durably persisted, or nothing is persisted and the packet remains `approved` — no `partially_ingested` state. Fails closed with SQLSTATE `GP422` on any present, unclassified, undispositioned allergen disclosure, with a structured payload (`evidence_type`, `missing_fields`, dish/restaurant context, `blocked_count`, `corrective_action_required`); the precondition applies only to disclosure entries that actually exist in `packet_data` — dishes with none impose no requirement. Calorie evidence is optional and never inferred; a dish with none ingests successfully with all calorie columns null. See `docs/decisions/DEC000003_CANONICAL_INTAKE_EVIDENCE_INGESTION_ARCHITECTURE.md` §3-§4. None of this is built — the additive schema migration, the approve-RPC precondition, the Evidence Disposition mechanism, and `commit_ingest` itself all remain unwritten (`docs/decisions/DEC000003_INTAKE_EVIDENCE_SCHEMA_GAP_MAP.md` §8, §10, §12).
...
**RESOLVED — Canonical Intake Evidence Ingestion Architecture (see DEC000003)**
The Blueprint implies Intake evidence flows into Supabase-backed Knowledge OS evidence tables. The current process writes to Google Sheets and later synchronizes generated files. **DEC000003 (`docs/decisions/DEC000003_CANONICAL_INTAKE_EVIDENCE_INGESTION_ARCHITECTURE.md`) was Founder-approved 2026-07-16** and resolves the policy/semantics layer of this question: `commit_ingest` transitions Supabase-direct, as a single all-or-nothing transaction — every approved, ingestion-bound evidence element must be durably persisted or the packet stays `approved`, with no dual-mode Sheets fallback and no `partially_ingested` state (DEC000003 §2-§3, interpreting DEC000001 §5.7 without amending it). Where a schema gap exists (e.g. `dish.verbatim_components[]`, `restaurant.source_inventory[]`, calorie sub-fields, allergen-disclosure classification), DEC000003 also settles how it must be closed — a proposed additive migration for the clean schema gaps, and a new Evidence Disposition mechanism (§4) plus a governed reopen invariant (§5) for the allergen-disclosure gap, which is an upstream Intake-model gap rather than a Supabase schema gap. **This resolves the destination question; it does not itself build the migration, the precondition, the Evidence Disposition mechanism, or `commit_ingest` — all remain unbuilt**, tracked in `docs/decisions/DEC000003_INTAKE_EVIDENCE_SCHEMA_GAP_MAP.md` §8, §10, §12. See §17's updated CMD000008 entry and §20 above. DEC000003 remains the governing authority if later wording drift occurs.
...
Create Decision Records for: (1) Canonical Intake Packet State Machine — **done, DEC000001, approved 2026-07-13**; (2) Canonical Submission State Machine — **done, DEC000002, approved 2026-07-18**; (3) Canonical Intake Evidence Ingestion Architecture — **done, DEC000003, approved 2026-07-16** (policy/semantics layer only — the additive schema migration, the approve-RPC precondition, the Evidence Disposition mechanism, and `commit_ingest` itself remain unbuilt, per `docs/decisions/DEC000003_INTAKE_EVIDENCE_SCHEMA_GAP_MAP.md` §12); (4) Review Flag Entity Model (DEC000004, still required). Priorities 1, 2, and (for policy) the ingestion-destination question are now approved; DEC000004 remains the one decision still blocking reliable command definitions elsewhere.
...
Implement `intake.packet.commit_ingest` — the most important remaining operation that requires leaving Master OS. **Per DEC000003 (approved 2026-07-16), the command is a single all-or-nothing transaction: success (every ingestion-bound evidence element durably persisted) or a closed-precondition failure (SQLSTATE `GP422`, packet stays `approved`, nothing written) — there is no partial-failure or partially-ingested outcome to distinguish, and no rollback/compensating-action design is needed since a failed precondition check never begins writing.** Build order per the gap map's sequencing plan (`docs/decisions/DEC000003_INTAKE_EVIDENCE_SCHEMA_GAP_MAP.md` §12): the additive schema migration first (decoupled), then the allergen-disclosure precondition track (approve-RPC precondition, Evidence Disposition mechanism, review-UI controls, tests), then `commit_ingest` itself.
